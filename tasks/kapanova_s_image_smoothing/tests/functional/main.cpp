...
class KapanovaSImageSmoothingFuncTests : public ppc::util::BaseRunFuncTests<InType, OutType, TestType> {
 public:
  static std::string PrintTestParam(const TestType &test_param) {
    int width_ = std::get<1>(test_param);
    int height_ = std::get<2>(test_param);

    std::string name = "image_" + std::to_string(width_) + "x" + std::to_string(height_);
 ...
 protected:
  void SetUp() override {
    ...
    width__ = std::get<1>(test_params_);
    height__ = std::get<2>(test_params_);
  ...
  bool CheckTestOutputData(OutType &output_data) final {
    ...
    if (expected_output_.empty()) {
      size_t expected_size = static_cast<size_t>(width__) * static_cast<size_t>(height__) * 3;
  ...
  InType GetTestinput_Data() final {
    ...
    data.push_back(static_cast<uint8_t>(width__ & 0xFF));
    data.push_back(static_cast<uint8_t>((width__ >> 8) & 0xFF));
    data.push_back(static_cast<uint8_t>(height__ & 0xFF));
    data.push_back(static_cast<uint8_t>((height__ >> 8) & 0xFF));
...
const int kwidth_3x3 = 3;
const int kheight_3x3 = 3;

const std::vector<uint8_t> kImage2x2 = {255, 0, 0, 0, 255, 0, 0, 0, 255, 255, 255, 255};
const int kwidth_2x2 = 2;
const int kheight_2x2 = 2;

const std::vector<uint8_t> kImage4x4(4 * 4 * 3, 128);
const int kwidth_4x4 = 4;
const int kheight_4x4 = 4;
...
const std::array<TestType, 3> kTestParam = {
    std::make_tuple(kImage3x3, kwidth_3x3, kheight_3x3, std::vector<uint8_t>()),
    std::make_tuple(kImage2x2, kwidth_2x2, kheight_2x2, std::vector<uint8_t>()),
    std::make_tuple(kImage4x4, kwidth_4x4, kheight_4x4, std::vector<uint8_t>())};
