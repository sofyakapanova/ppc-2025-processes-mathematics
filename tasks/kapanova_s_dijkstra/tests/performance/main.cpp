...
TEST_F(KapanovaSDijkstraPerfTests, MediumRandomGraphSEQ) {
  GraphData graph = GenerateRandomGraph(3000, 0.05);
  RunSeqPerformanceTest(graph, "Medium Random Graph SEQ (3000 vertices)");
}
...
TEST_F(KapanovaSDijkstraPerfTests, MediumRandomGraphMPI) {
  GraphData graph = GenerateRandomGraph(3000, 0.05);
  RunMPIPerformanceTest(graph, "Medium Random Graph MPI (3000 vertices)");
}
