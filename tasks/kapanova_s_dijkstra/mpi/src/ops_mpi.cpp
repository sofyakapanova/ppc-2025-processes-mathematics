...
namespace {
void ComputeNodeDistribution(int rank, int size, int total_nodes, 
                             int &local_first, int &local_last, int &local_nodes) {
...
void LoadLocalEdges(const GraphData &graph, int local_first, int local_last, 
                    int total_nodes, std::vector<int> &local_cols, 
                    std::vector<double> &local_vals, std::vector<int> &local_rows) {
  if (local_last > total_nodes) local_last = total_nodes;
  ...
  if (edge_count > 0) {
    ...
    std::copy(graph.col_idx.begin() + first_edge, 
              graph.col_idx.begin() + last_edge, 
              local_cols.begin());
    std::copy(graph.weights.begin() + first_edge, 
              graph.weights.begin() + last_edge, 
              local_vals.begin());
  } else {
    local_cols.clear();
    local_vals.clear();
...
bool RunOptimizedDijkstraMPI(const std::vector<int> &local_rows,
                            const std::vector<int> &local_cols,
                            const std::vector<double> &local_vals,
                            int local_first, int local_nodes, int total_nodes,
  ...
  while (any_changed && iterations < total_nodes) {
    ...
    for (int local_idx = 0; local_idx < local_nodes; ++local_idx) {
      ...
      if (u >= total_nodes) continue;
      
      double dist_u = current_dist[u];
      if (dist_u == std::numeric_limits<double>::infinity()) continue;
      
      if (local_idx >= static_cast<int>(local_rows.size()) - 1) continue;
      
      int start = local_rows[local_idx];
      int end = local_rows[local_idx + 1];
      
      for (int edge_idx = start; edge_idx < end; ++edge_idx) {
        if (static_cast<size_t>(edge_idx) >= local_cols.size()) continue;
        
        int v = local_cols[edge_idx];
        if (v < 0 || v >= total_nodes) continue;
    ...
    MPI_Allreduce(MPI_IN_PLACE, next_dist.data(), total_nodes, 
    ...
    int all_changed = 0;
    MPI_Allreduce(&global_changed, &all_changed, 1, 
...
void FastInitializeDistances(const GraphData &graph, int rank, int size,
                            std::vector<double> &global_dist, int total_nodes) {
...
void KapanovaSDijkstraMPI::PartitionGraph() {
  ...
  int local_first = 0;
  int local_last = 0;
  ComputeNodeDistribution(proc_rank_, proc_count_, total_nodes, 
                         local_first, local_last, local_vertices_);
  ...
  LoadLocalEdges(graph, local_first, local_last, total_nodes,
                 local_col_idx_, local_weights_, local_row_ptr_);
}
...
bool KapanovaSDijkstraMPI::RunImpl() {
  ...
  int local_nodes = 0;
  ComputeNodeDistribution(proc_rank_, proc_count_, total_nodes, 
                         local_first, local_last, local_nodes);
  
  if (local_nodes != local_vertices_) {
    local_vertices_ = local_nodes;
  }
  
  RunOptimizedDijkstraMPI(local_row_ptr_, local_col_idx_, local_weights_,
                          local_first, local_vertices_, total_nodes,
                          global_dist);
